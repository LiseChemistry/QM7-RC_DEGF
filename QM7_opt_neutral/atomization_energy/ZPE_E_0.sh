...
while IFS= read -r molname; do
    logfile="/home/student5/lise/MasterProject_SPAHM-ENN/QM7_opt_neutral/opt_neutral_xyz/log_opt_0_1/${molname}.log"
    if [[ -f "$logfile" ]]; then	
	export string=$(grep "SCF Done:  E" "$logfile" | tail -n 1)
